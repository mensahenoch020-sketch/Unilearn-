...
export default function App() {
  ...
  const checkEnrollments = async (profile) => {
    ...
    try {
      ...
      // Note: Admins do not need enrollment checks
    } catch (err) {
  ...
  const handleLogin = async (profile) => {
    ...
    // If the student arrived here via a QR attendance scan (and had to
    // log in first), send them back to that exact attendance link now
    // that they're authenticated, instead of dropping them on the
    // regular dashboard and silently losing the scan.
