      ...
      <div style={{animation:"fadeIn 0.5s ease",width:"100%",maxWidth:400}}>
        ...
        <div style={{background:"#fff",borderRadius:24,padding:28,boxShadow:"0 20px 60px rgba(0,0,0,0.15)"}}>
          ...
          <button onClick={mode==="login"?handleLogin:handleSignup} disabled={loading}
            style={{background:"#1B4332",color:"#fff",border:"none",borderRadius:12,padding:"15px 0",width:"100%",fontSize:15,fontWeight:700,cursor:"pointer",opacity:loading?0.7:1,fontFamily:"Inter,sans-serif",transition:"opacity 0.2s"}}>
    ...
    <div style={{animation:"fadeIn 0.4s ease"}}>
      ...
      <div style={{background:`linear-gradient(135deg,${C.primary},${C.primaryLight})`,borderRadius:20,padding:24,marginBottom:20,color:"#fff",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{display:"flex",gap:10,marginTop:16,flexWrap:"wrap"}}>
          <div style={{background:"rgba(255,255,255,0.15)",borderRadius:10,padding:"6px 14px",fontSize:12,fontWeight:600,backdropFilter:"blur(10px)"}}>{courses.length} Courses</div>
          <div style={{background:"rgba(255,255,255,0.15)",borderRadius:10,padding:"6px 14px",fontSize:12,fontWeight:600,backdropFilter:"blur(10px)"}}>2024/2025</div>
          <div style={{background:"rgba(255,255,255,0.15)",borderRadius:10,padding:"6px 14px",fontSize:12,fontWeight:600,backdropFilter:"blur(10px)"}}>Harmattan Semester</div>
        </div>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:24}}>
        {[
          ...
          <div key={i} onClick={()=>s.tab&&setTab(s.tab)} style={{background:C.card,borderRadius:16,padding:16,border:`1px solid ${C.border}`,cursor:s.tab?"pointer":"default",transition:"transform 0.2s",active:{transform:"scale(0.98)"}}}>
        ...
        <div key={qi} style={{background:C.card,borderRadius:16,padding:20,marginBottom:14,border:`1px solid ${C.border}`}}>
          ...
          {q.options.map((opt,oi)=>(
            <div key={oi} onClick={()=>setAnswers({...answers,[qi]:oi})}
              ...
              <div style={{width:20,height:20,borderRadius:"50%",border:`2px solid ${answers[qi]===oi?C.primary:"#ccc"}`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,transition:"all 0.15s"}}>
    ...
    return (
      <div style={{textAlign:"center",paddingTop:40,animation:"fadeIn 0.4s ease"}}>
        ...
        <div style={{background:C.card,borderRadius:20,padding:24,margin:"24px 0",border:`1px solid ${C.border}`}}>
          ...
          <div style={{marginTop:16,fontSize:14,color:C.muted}}>{pct===100?"Perfect score! Outstanding work 🌟":pct>=70?"Great job! Keep it up 💪":pct>=50?"You passed. Review weak areas 📖":"Keep studying. You can do better! 💡"}</div>
        </div>
    ...
    <div>
      ...
      {courses.map((c,i)=>(
        <div key={c.id} onClick={()=>setSelected(c)}
          style={{background:C.card,borderRadius:16,padding:16,marginBottom:10,border:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:14,cursor:"pointer",animation:`fadeIn 0.4s ease ${i*0.05}s both`,transition:"transform 0.15s"}}>
...
function Grades({user,C}) {
  ...
  useEffect(()=>{
    if(loading) return;
    let start=0; const target=cgpa; const duration=1500;
  ...
  const cgpaColor = cgpa>=4.5?"#10B981":cgpa>=3.5?"#3B82F6":cgpa>=2.5?"#F59E0B":"#EF4444";
      ...
      {grades.map(g=>{
        ...
        return(
          <div key={g.id} style={{background:C.card,borderRadius:16,padding:16,marginBottom:12,border:`1px solid ${C.border}`}}>
            ...
            {total!==null&&<div>
              <div style={{background:C.border,borderRadius:10,height:6,overflow:"hidden"}}>
                <div style={{background:grade?.color,height:"100%",width:`${pct}%`,borderRadius:10,transition:"width 1s ease"}}/>
              </div>
            </div>}
          </div>
        );
...
function Settings({user,setUser,C,onLogout}) {
  ...
  const saveName = async()=>{
    ...
    if(e){setError(e.message);}else{setUser({...user,name});setMessage("Name updated successfully!");}
    setLoading(false);
  };

  const changePassword = async()=>{
    if(!newPw||!confirmPw) return setError("Please fill in all password fields.");
    ...
    if(e){setError(e.message);}else{setMessage("Password changed successfully!");setCurrentPw("");setNewPw("");setConfirmPw("");}
    setLoading(false);
  };
    ...
    <div>
      ...
      <div style={{background:C.card,borderRadius:20,padding:24,marginBottom:16,border:`1px solid ${C.border}`,textAlign:"center"}}>
        <div style={{position:"relative",width:96,height:96,margin:"0 auto 16px"}}>
          ...
          <label style={{position:"absolute",bottom:0,right:0,background:C.accent,borderRadius:"50%",width:32,height:32,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",boxShadow:"0 2px 8px rgba(0,0,0,0.2)"}}>
      ...
      <div style={{background:C.card,borderRadius:16,padding:20,marginBottom:16,border:`1px solid ${C.border}`}}>
        ...
        <div style={{marginBottom:16}}><label style={{fontSize:12,fontWeight:600,color:C.muted}}>Confirm New Password</label><input style={inp} type="password" value={confirmPw} onChange={e=>setConfirmPw(e.target.value)} placeholder="••••••••"/></div>
    ...
    <div>
      <div style={{background:C.card,borderRadius:20,padding:20,marginBottom:20,border:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:16}}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{fontSize:12,color:C.muted}}>{user?.role} · {user?.matric}</div>
        </div>
      </div>

      {items.map(it=>(
        <div key={it.tab} onClick={()=>setTab(it.tab)}
          style={{background:C.card,borderRadius:16,padding:16,marginBottom:10,border:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:14,cursor:"pointer",transition:"transform 0.15s"}}>
      ...
      <div style={{background:C.card,borderRadius:16,padding:16,marginBottom:16,border:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:14}}>
        ...
        <div style={{flex:1}}>
          ...
          <div style={{fontSize:12,color:C.muted}}>Switch appearance</div>
        </div>
        <div onClick={()=>setDark(!dark)} style={{width:50,height:28,borderRadius:14,background:dark?C.primary:"#E8E8E8",cursor:"pointer",position:"relative",transition:"background 0.3s",flexShrink:0}}>
          <div style={{position:"absolute",top:4,left:dark?26:4,width:20,height:20,borderRadius:"50%",background:"#fff",transition:"left 0.3s",boxShadow:"0 1px 4px rgba(0,0,0,0.2)"}}/>
        </div>
      </div>
    </div>
  );
...
function CallScreen({callType,onClose,C}) {
  ...
  if(roomUrl) return (
    <div style={{position:"fixed",inset:0,background:"#000",zIndex:999,display:"flex",flexDirection:"column"}}>
      <div style={{display:"flex",alignItems:"center",padding:"14px 18px",background:"rgba(0,0,0,0.9)",backdropFilter:"blur(10px)"}}>
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"#0D1B2A",zIndex:999,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:32}}>
      ...
      <div style={{width:110,height:110,borderRadius:"50%",background:"linear-gradient(135deg,#1B4332,#F4A261)",display:"flex",alignItems:"center",justifyContent:"center",animation:"pulse 2s ease infinite",boxShadow:"0 0 40px rgba(27,67,50,0.5)"}}>
        <Ic n={callType==="video"?"video":"phone"} s={44} c="#fff" w={2}/>
      </div>
      <div style={{display:"flex",gap:16}}>
        {callType==="video"&&<button onClick={()=>setCamOff(!camOff)} style={{width:56,height:56,borderRadius:"50%",background:camOff?"#EF4444":"rgba(255,255,255,0.15)",border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(10px)"}}>
          <Ic n={camOff?"videoOff":"video"} s={22} c="#fff"/>
        </button>}
        <button onClick={()=>setMuted(!muted)} style={{width:56,height:56,borderRadius:"50%",background:muted?"#EF4444":"rgba(255,255,255,0.15)",border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(10px)"}}>
          <Ic n={muted?"micOff":"mic"} s={22} c="#fff"/>
        </button>
        <button onClick={onClose} style={{width:56,height:56,borderRadius:"50%",background:"#EF4444",border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 16px rgba(239,68,68,0.4)"}}>
  ...
  return (
    <div>
      ...
      <div style={{background:"#FEF3C7",borderRadius:14,padding:16,border:"1px solid #FDE68A",marginTop:8}}>
        ...
        <div style={{fontSize:12,color:"#92400E",lineHeight:1.6}}>Students must maintain a minimum of 75% attendance in all courses to be eligible to sit for end-of-semester examinations. Students below this threshold may be barred from writing the exam.</div>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  return (
    <div style={{fontFamily:"'Inter',sans-serif",background:C.bg,minHeight:"100vh",maxWidth:480,margin:"0 auto",position:"relative"}}>
      ...
      `}</style>
      <div style={{background:C.headerBg,color:C.headerText,padding:"14px 20px",display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:0,zIndex:100,backdropFilter:"blur(20px)"}}>
