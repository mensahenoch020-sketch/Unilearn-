...
export default function Dashboard({ user, C }) {
  ...
  // Fix: properly display faculty
  const facultyDisplay = user?.faculty
    ? user.faculty.toUpperCase()
    : user?.department?.toUpperCase() || "STUDENT";
  ...
  return (
    <div style={{ animation: "fadeIn 0.4s ease" }}>
      ...
      <div style={{
        ...
        borderRadius: 20, padding: "24px 20px", marginBottom: 20, color: "#fff",
        position: "relative", overflow: "hidden", boxShadow: "0 8px 24px rgba(27,67,50,0.25)",
      }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 24 }}>
        {quickStats.map((s) => (
          <div key={s.label} onClick={() => navigate(s.path)}
            style={{ background: C.card, borderRadius: 16, padding: 16, border: `1px solid ${C.border}`, cursor: "pointer", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
      ...
      {enrolledCourses.length === 0 && (
        <div style={{ background: C.card, borderRadius: 16, padding: 24, marginBottom: 20, border: `1px solid ${C.border}`, textAlign: "center", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
          <div style={{ fontSize: 40, marginBottom: 12 }}>📚</div>
          <div style={{ fontWeight: 700, fontSize: 16, color: C.text, marginBottom: 8 }}>Add your courses</div>
          <div style={{ color: C.muted, fontSize: 13, marginBottom: 16 }}>Go to Courses tab to enroll</div>
          <button onClick={() => navigate("/courses")} style={{ background: C.primary, color: "#fff", border: "none", borderRadius: 12, padding: "12px 24px", fontWeight: 700, cursor: "pointer", fontFamily: "Inter,sans-serif" }}>Browse Courses</button>
        </div>
      )}
      ...
      {/* Announcements with lecturer name */}
      {announcements.length > 0 && (
        <>
          <div style={{ fontSize: 11, fontWeight: 700, marginBottom: 12, marginTop: 8, color: C.muted, letterSpacing: 0.8 }}>📢 ANNOUNCEMENTS</div>
          {announcements.map((a) => (
            <div key={a.id} style={{ background: C.card, borderRadius: 16, padding: 16, marginBottom: 12, border: `1px solid ${C.border}`, boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6, paddingTop: 10, borderTop: `1px solid ${C.border}` }}>
                <div style={{ width: 20, height: 20, borderRadius: "50%", background: "#1B433220", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <Ic n="user" s={11} c="#1B4332" />
                </div>
                <span style={{ fontSize: 11, color: C.muted, fontWeight: 600 }}>{a.profiles?.name || "Lecturer"}</span>
                <span style={{ fontSize: 11, color: C.muted }}>· {new Date(a.created_at).toLocaleDateString("en-NG", { day: "numeric", month: "short" })}</span>
              </div>
            </div>
          ))}
        </>
      ...
      {enrolledCourses.length > 0 && (
        <>
          <div style={{ fontSize: 11, fontWeight: 700, marginBottom: 12, marginTop: 8, color: C.muted, letterSpacing: 0.8 }}>MY COURSES</div>
          {enrolledCourses.map((c) => (
            <div key={c.id} onClick={() => navigate("/courses")}
              style={{ background: C.card, borderRadius: 16, padding: 14, marginBottom: 10, border: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 12, cursor: "pointer", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
              <div style={{ width: 44, height: 44, background: (c.color || "#1B4332") + "15", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center" }}>
